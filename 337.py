...
class Solution:
    ...
    def rob(self, root: TreeNode) -> int:
        def helper(node):
            """返回在以 node 为根节点的二叉树上, 偷这个节点 和 不偷这个节点 的最大抢劫金额"""
            if node is None:
                return 0, 0
            ret_left = helper(node.left)
            ret_right = helper(node.right)
            return node.val + ret_left[1] + ret_right[1], max(ret_left) + max(ret_right)
...
root = construct_tree([3, 2, 3, None, 3, None, 1])
root = construct_tree([3, 4, 5, 1, 3, None, 1])
