...
class Solution:
    ...
    def PredictTheWinner_1(self, nums: List[int]) -> bool:
        ...
        size_nums = len(nums)
        dp = [[[0, 0] for idx in range(size_nums)] for jdx in range(size_nums)]
        ...
        # print(dp[0][size_nums-1])
        return dp[0][size_nums-1][0] >= dp[0][size_nums-1][1]

    # 另一种动态规划
    def PredictTheWinner(self, nums: List[int]) -> bool:
        # dp[i][j] 表示 piles[i, ..., j] 中 当前玩家和另一个玩家分数之差的最大值
        size_nums = len(nums)
        dp = [[0] * size_nums for _ in range(size_nums)]
        for i in range(size_nums):
            dp[i][i] = nums[i]
        for i in range(size_nums-1, -1, -1):
            for j in range(i+1, size_nums):
